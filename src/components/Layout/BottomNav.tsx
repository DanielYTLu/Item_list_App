  ...
  const renderLink = (item: any) => (
    <NavLink
      ...
      className={({ isActive }) =>
        ...
        } ${item.isCenter ? '-mt-6 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-md border-4 border-cream' : ''}`
      }
    >
      <item.icon size={item.isCenter ? 26 : 22} />
      {!item.isCenter && <span className="text-[11px] mt-1 tracking-tight">{item.label}</span>}
    </NavLink>
  );
